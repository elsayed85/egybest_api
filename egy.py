...
class EgyGrab():
    ...
    def grab(self, quality='1080p', cookies=True):
        ...
        if self.type in [TYPES.series, TYPES.anime, TYPES.wwe, TYPES.show]:
            ...
            urls = reversed(['https://tool.egybest.zone' + match[1]
        ...
        for url in urls:
            ...
            for match in re.finditer(r'(\/episode\/.+?)"', html):
                t = threading.Thread(target=lambda: season_results.append(
                    self.__grab_item('https://tool.egybest.zone' + match[1], quality, cookies, i)))
                i += 1
                t.start()
    ...
    def __grab_item(self, url, quality='1080p', last_session=True, id=0):
        try:
            ...
            # call = 'https://tool.egybest.zone/api?call='+re.search(r'rel="#yt_trailer".+?data-call="(.+?)"', html)[1]
            ...
            if not code:
                ...
                return (id, '')
            code = code[1]
            # watch = 'https://tool.egybest.zone'+re.search(r'<iframe.+?src="(.+?)"', html)[1]
            name = re.search(
            ...
            vidstream = sess.get(
                'https://tool.egybest.zone'+code, allow_redirects=False).headers['location']
            debug(vidstream)
            if vidstream[:2] == '/?':
                verification = 'https://tool.egybest.zone/api?call=' + \
                    (''.join([data[b][v] for v in data[a] if v in data[b]]))
                click = 'https://tool.egybest.zone/' + \
                    base64.b64decode(
                ...
                debug('vidstream link grabbed using cookies')
                pass
            # vidstream = 'https://tool.egybest.zone/vs-mirror/vidstream.to/f/YvD0EpU8nU'
            vidstream2 = vidstream
            vidstream = 'https://tool.egybest.zone/vs-mirror/' + \
                re.search(r'vidstream.+?\/f\/.+?\/', vidstream)[0]
            # print(vidstream)
            debug(vidstream2)
            ...
            if not re.search('<a href="(h.+?)" class="bigbutton"', html):
                ...
                # print(token)
                sess.get('https://tool.egybest.zone/' +
                         base64.b64decode(token + '===').decode('utf8'))
                verification = ''.join(
                    [values[keys[i]] for i in range(len(keys)) if keys[i] in values])
                # time.sleep(.2)
                sess.post('https://tool.egybest.zone/tvc.php?verify='+verification,
                          data={re.search(r"'([^']+?)':'ok'", html)[1]: 'ok'})
                html = sess.get(vidstream).text
            else:
                debug('download link grabbed using cookies')
                pass
            ...
            rrr = re.search('<a href="(h.+?)" class="bigbutton"', html)
            debug(rrr , rrr[0] , rrr[1])
            return (id, {
                # 'title': title,
                # 'cover': cover,
                'download' : re.search('<a href="(h.+?)" class="bigbutton"', html)[1]
            })
...
# parser.add_argument(
#     'url', help='an Egybest url of a movie, episode, season, or series. should start with https://tool.egybest.zone/')
parser.add_argument('-q', '--quality', type=str,
                    default='1080p', help='the desired quality')
parser.add_argument('-C', '--no-cookies', dest='cookies', action='store_false',
                    help='creates a new session for each grabbing request')

args = parser.parse_args()

# args.url = 'https://tool.egybest.zone/series/new-amsterdam/?ref=home-tv'
# args.quality = '1080p'
args.url = "https://tool.egybest.zone/movie/avatar-the-way-of-water-2022"
grabber = EgyGrab(args.url)

print('Your URL links to "%s"' % grabber.type.name)

filename = re.search(r'.+?\/%s\/(.+?)(\/|$)' %
                     grabber.type.value, args.url)[1] + '.txt'

debug(grabber.grab(args.quality))
